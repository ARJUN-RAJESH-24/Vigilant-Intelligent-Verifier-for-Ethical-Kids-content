...
def load_data():
    ...
    # Merge all feature dataframes with labels
    df = text_df.merge(audio_df, on="id", how="inner", suffixes=('', '_audio')) \
                .merge(video_df, on="id", how="inner", suffixes=('', '_video')) \
                .merge(labels_df, on="id", how="inner")
    ...
    if missing_counts.sum() > 0:
        print(f"\n⚠️  Found {missing_counts.sum()} missing values, imputing with mean...")
...
def main():
    ...
    try:
        ...
        print("\n   Alternatively, run 'python demo.py' for a complete end-to-end demonstration.")
